...
**Fastest path:** open [migrations/_bundle.sql](migrations/_bundle.sql), copy the whole file, paste into Supabase dashboard → **SQL Editor → New query → Run**. That single paste applies every migration in order (the bundle is regenerated by `node supabase/scripts/generate-bundle.mjs` — its header states the current range; never hand-edit it). Should see "Success. No rows returned." Afterwards, run the `validate constraint` blocks noted at the top of 0032/0033, and read [CLIENT_CONTRACT.md](CLIENT_CONTRACT.md) for what the client is allowed to query.
